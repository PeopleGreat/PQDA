...
def make_env(
	...
	assert mode in {'train', 'color_easy', 'color_hard', 'video_easy', 'video_hard', 
				'color_easy_video_easy',
				'color_hard_video_easy',
				'color_easy_video_hard',
				'color_hard_video_hard',
				'distracting_cs'}, \
	...
	if 'video' in mode and 'color' in mode:
		env = VideoColorWrapper(env, mode, seed)
...
class VideoColorWrapper(gym.Wrapper):
	...
	def reset(self):
		...
		# obs = self.env.reset()
		obs = self._greenscreen(self.env.reset())
		self.frames.clear()
	...
	def _load_colors(self):
		assert 'color_easy' in self._mode or 'color_hard' in self._mode
		if 'color_easy' in self._mode:
			color_type = 'color_easy'
		elif 'color_hard' in self._mode:
			color_type = 'color_hard'
		self._colors = torch.load(f'./data/{color_type}.pt')

	def get_random_color(self):
		assert len(self._colors) >= 100, 'env must include at least 100 colors'

		colors = self._colors[self._random_state.randint(len(self._colors))]
		setting_kwargs = {
				'skybox_rgb': [.2, .8, .2],
				'skybox_rgb2': [.2, .8, .2],
				'skybox_markrgb': [.2, .8, .2],
				'grid_rgb1': colors['grid_rgb1'],
				'grid_rgb2': colors['grid_rgb2'],
				'self_rgb': colors['self_rgb'],
				'skybox_rgb': colors['skybox_rgb'],
		}
		if 'video_hard' in self._mode:
	...
	def _get_video_paths(self):
		# video_dir = os.path.join('/home/wuyq/ERG/src/env/data', self._mode)
		if 'video_easy' in self._mode:
			self._video_paths = [os.path.join('./data', f'video_easy/video{i}.mp4') for i in range(10)]
		elif 'video_hard' in self._mode:
			self._video_paths = [os.path.join('./data', f'video_hard/video{i}.mp4') for i in range(100)]
		else:
...
class ColorWrapper(gym.Wrapper):
	...
	def reset(self):
		...
		self.frames.clear()
		frame = np.array(obs)
		self.frames.append(np.transpose(frame, (1, 2, 0)))
		return obs
	...
	def _load_colors(self):
		...
		self._colors = torch.load(f'./data/{self._mode}.pt')
...
class VideoWrapper(gym.Wrapper):
	...
	def _get_video_paths(self):
		video_dir = os.path.join('./data', self._mode)
		if 'video_easy' in self._mode:
